...
export const columns: ColumnDef<InventoryTableProps>[] = [
    ...
    cell: ({ row }) => {
      return (
        <div className="flex flex-col pl-1">
          ...
          <span className="text-xs text-muted-foreground">PROD003</span>
        </div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Tag text={category} color={category === "Venta" ? "blue" : "purple"} />
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return stock < minStock ? (
        <Tooltip>
          ...
          <TooltipContent>
            <p>Stock bajo el mínimo recomendado: {minStock}</p>
          </TooltipContent>
        </Tooltip>
      ) : (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const lastMovement = row.original.lastMovement;
      return (
        <span>
          {lastMovement.toLocaleDateString("es-ES", {
            day: "2-digit",
            month: "short",
            year: "2-digit",
          })}
        ...
        <>
          <div className="justify-self-end">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="border bg-card rounded-lg p-1 shadow-lg"
              >
                <DropdownMenuItem
